; See constants/evolution_constants.asm
; The max number of evolutions per monster is MAX_EVOLUTIONS

EvosMovesPointerTable:
	dw RhydonEvosMoves
	dw KangaskhanEvosMoves
	dw NidoranMEvosMoves
	dw ClefairyEvosMoves
	dw SpearowEvosMoves
	dw VoltorbEvosMoves
	dw NidokingEvosMoves
	dw SlowbroEvosMoves
	dw IvysaurEvosMoves
	dw ExeggutorEvosMoves
	dw LickitungEvosMoves
	dw ExeggcuteEvosMoves
	dw GrimerEvosMoves
	dw GengarEvosMoves
	dw NidoranFEvosMoves
	dw NidoqueenEvosMoves
	dw CuboneEvosMoves
	dw RhyhornEvosMoves
	dw LaprasEvosMoves
	dw ArcanineEvosMoves
	dw MewEvosMoves
	dw GyaradosEvosMoves
	dw ShellderEvosMoves
	dw TentacoolEvosMoves
	dw GastlyEvosMoves
	dw ScytherEvosMoves
	dw StaryuEvosMoves
	dw BlastoiseEvosMoves
	dw PinsirEvosMoves
	dw TangelaEvosMoves
	dw CrobatEvosMoves			;NEW
	dw MareepEvosMoves			;NEW
	dw GrowlitheEvosMoves
	dw OnixEvosMoves
	dw FearowEvosMoves
	dw PidgeyEvosMoves
	dw SlowpokeEvosMoves
	dw KadabraEvosMoves
	dw GravelerEvosMoves
	dw ChanseyEvosMoves
	dw MachokeEvosMoves
	dw MrMimeEvosMoves
	dw HitmonleeEvosMoves
	dw HitmonchanEvosMoves
	dw ArbokEvosMoves
	dw ParasectEvosMoves
	dw PsyduckEvosMoves
	dw DrowzeeEvosMoves
	dw GolemEvosMoves
	dw FlaaffyEvosMoves			;NEW
	dw MagmarEvosMoves
	dw AmpharosEvosMoves		;NEW
	dw ElectabuzzEvosMoves
	dw MagnetonEvosMoves
	dw KoffingEvosMoves
	dw BellossomEvosMoves		;NEW
	dw MankeyEvosMoves
	dw SeelEvosMoves
	dw DiglettEvosMoves
	dw TaurosEvosMoves
	dw SudowoodoEvosMoves		;NEW
	dw PolitoedEvosMoves		;NEW
	dw HoppipEvosMoves			;NEW
	dw FarfetchdEvosMoves
	dw VenonatEvosMoves
	dw DragoniteEvosMoves
	dw SkiploomEvosMoves		;NEW
	dw JumpluffEvosMoves		;NEW
	dw LeafeonEvosMoves			;NEW
	dw DoduoEvosMoves
	dw PoliwagEvosMoves
	dw JynxEvosMoves
	dw MoltresEvosMoves
	dw ArticunoEvosMoves
	dw ZapdosEvosMoves
	dw DittoEvosMoves
	dw MeowthEvosMoves
	dw KrabbyEvosMoves
	dw WooperEvosMoves			;NEW
	dw QuagsireEvosMoves		;NEW
	dw DunsparceEvosMoves		;NEW
	dw VulpixEvosMoves
	dw NinetalesEvosMoves
	dw PikachuEvosMoves
	dw RaichuEvosMoves
	dw MisdreavusEvosMoves		;NEW
	dw SlowkingEvosMoves		;NEW
	dw DratiniEvosMoves
	dw DragonairEvosMoves
	dw KabutoEvosMoves
	dw KabutopsEvosMoves
	dw HorseaEvosMoves
	dw SeadraEvosMoves
	dw GligarEvosMoves			;NEW
	dw DimonixEvosMoves			;NEW
	dw SandshrewEvosMoves
	dw SandslashEvosMoves
	dw OmanyteEvosMoves
	dw OmastarEvosMoves
	dw JigglypuffEvosMoves
	dw WigglytuffEvosMoves
	dw EeveeEvosMoves
	dw FlareonEvosMoves
	dw JolteonEvosMoves
	dw VaporeonEvosMoves
	dw MachopEvosMoves
	dw ZubatEvosMoves
	dw EkansEvosMoves
	dw ParasEvosMoves
	dw PoliwhirlEvosMoves
	dw PoliwrathEvosMoves
	dw WeedleEvosMoves
	dw KakunaEvosMoves
	dw BeedrillEvosMoves
	dw QwilfishEvosMoves		;NEW
	dw DodrioEvosMoves
	dw PrimeapeEvosMoves
	dw DugtrioEvosMoves
	dw VenomothEvosMoves
	dw DewgongEvosMoves
	dw ScizorEvosMoves			;NEW
	dw HeracrossEvosMoves		;NEW
	dw CaterpieEvosMoves
	dw MetapodEvosMoves
	dw ButterfreeEvosMoves
	dw MachampEvosMoves
	dw SneaselEvosMoves			;NEW
	dw GolduckEvosMoves
	dw HypnoEvosMoves
	dw GolbatEvosMoves
	dw MewtwoEvosMoves
	dw SnorlaxEvosMoves
	dw MagikarpEvosMoves
	dw TeddiursaEvosMoves		;NEW
	dw UrsaringEvosMoves		;NEW
	dw MukEvosMoves
	dw SlugmaEvosMoves			;NEW
	dw KinglerEvosMoves
	dw CloysterEvosMoves
	dw MagcargoEvosMoves		;NEW
	dw ElectrodeEvosMoves
	dw ClefableEvosMoves
	dw WeezingEvosMoves
	dw PersianEvosMoves
	dw MarowakEvosMoves
	dw SwinubEvosMoves			;NEW
	dw HaunterEvosMoves
	dw AbraEvosMoves
	dw AlakazamEvosMoves
	dw PidgeottoEvosMoves
	dw PidgeotEvosMoves
	dw StarmieEvosMoves
	dw BulbasaurEvosMoves
	dw VenusaurEvosMoves
	dw TentacruelEvosMoves
	dw PiloswineEvosMoves		;NEW
	dw GoldeenEvosMoves
	dw SeakingEvosMoves
	dw CorsolaEvosMoves			;NEW
	dw HoundourEvosMoves		;NEW
	dw HoundoomEvosMoves		;NEW
	dw KingdraEvosMoves			;NEW
	dw PonytaEvosMoves
	dw RapidashEvosMoves
	dw RattataEvosMoves
	dw RaticateEvosMoves
	dw NidorinoEvosMoves
	dw NidorinaEvosMoves
	dw GeodudeEvosMoves
	dw PorygonEvosMoves
	dw AerodactylEvosMoves
	dw PhanpyEvosMoves			;NEW
	dw MagnemiteEvosMoves
	dw DonphanEvosMoves			;NEW
	dw Porygon2EvosMoves		;NEW
	dw CharmanderEvosMoves
	dw SquirtleEvosMoves
	dw CharmeleonEvosMoves
	dw WartortleEvosMoves
	dw CharizardEvosMoves
	dw LickilickiEvosMoves		;NEW
	dw ReefetchdEvosMoves		;NEW
	dw LugiaEvosMoves			;NEW
	dw HoohEvosMoves			;NEW
	dw OddishEvosMoves
	dw GloomEvosMoves
	dw VileplumeEvosMoves
	dw BellsproutEvosMoves
	dw WeepinbellEvosMoves
	dw VictreebelEvosMoves
	dw ArmMewtwoEvosMoves		;NEW
	dw VenustoiseEvosMoves		;NEW
	dw MissingNoC1EvosMoves		;readded, unlisted, replacing MissingNoB5EvosMoves
	dw FossilKabutopsEvosMoves	;readded, unlisted
	dw FossilAerodactylEvosMoves;readded, unlisted
	dw MonGhostEvosMoves		;readded, unlisted

RhydonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, ROAR
	db 30, STOMP
	db 35, TAIL_WHIP
	db 40, FURY_ATTACK
	db 48, HORN_DRILL
	db 55, LEER
	db 64, TAKE_DOWN
	db 0

KangaskhanEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, ROAR
	db 16, STOMP
	db 21, HEADBUTT
	db 26, BITE
	db 31, TAIL_WHIP
	db 36, MEGA_PUNCH
	db 41, LEER
	db 46, DIZZY_PUNCH
	db 51, FOCUS_ENERGY
	db 56, DISABLE
	db 0

NidoranMEvosMoves:
; Evolutions
	db EV_LEVEL, 16, NIDORINO
	db 0
; Learnset
	db 8, HORN_ATTACK
	db 14, POISON_STING
	db 21, FOCUS_ENERGY
	db 27, SUPERSONIC
	db 29, FURY_ATTACK
	db 36, LOVELY_KISS
	db 36, HORN_DRILL
	db 43, DOUBLE_KICK
	db 0

ClefairyEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, CLEFABLE
	db 0
; Learnset
	db 13, SING
	db 18, DOUBLESLAP
	db 24, MINIMIZE
	db 31, METRONOME
	db 38, DIZZY_PUNCH
	db 39, DEFENSE_CURL
	db 42, DREAM_EATER
	db 48, LIGHT_SCREEN
	db 0

SpearowEvosMoves:
; Evolutions
	db EV_LEVEL, 20, FEAROW
	db 0
; Learnset
	db 9, LEER
	db 15, FURY_ATTACK
	db 19, SONICBOOM
	db 22, MIRROR_MOVE
	db 24, SLAM
	db 29, DRILL_PECK
	db 29, DRILL_PECK
	db 36, AGILITY
	db 43, TRI_ATTACK
	db 0

VoltorbEvosMoves:
; Evolutions
	db EV_LEVEL, 30, ELECTRODE
	db 0
; Learnset
	db 17, SONICBOOM
	db 19, THUNDERSHOCK							;Added, so Voltorb is viable against Water-types early-game
	db 20, THUNDER_WAVE							;Added, for wild Voltorbs in Route 4
	db 22, SELFDESTRUCT
	db 29, LIGHT_SCREEN
	db 36, SWIFT
	db 39, BARRAGE
	db 43, EXPLOSION
	db 44, AMNESIA
	db 66, THUNDERBOLT							;Added, for wild Voltorbs in Silph Co.
	db 66, TAKE_DOWN							;Added, for wild Voltorbs in Silph Co.
	db 66, THUNDER_WAVE							;Added, for wild Voltorbs in Silph Co.
	db 66, EXPLOSION							;Added, for wild Voltorbs in Silph Co.
	db 0

NidokingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, HORN_ATTACK
	db 14, POISON_STING
	db 23, THRASH
	db 27, FIRE_PUNCH
	db 28, THUNDERPUNCH
	db 29, ICE_PUNCH
	db 0

SlowbroEvosMoves:
; Evolutions
	db 0
; Learnset
	db 18, DISABLE
	db 22, HEADBUTT
	db 27, GROWL
	db 33, WATER_GUN
	db 37, WITHDRAW
	db 44, AMNESIA
	db 55, PSYCHIC_M
	db 0

IvysaurEvosMoves:
; Evolutions
	db EV_LEVEL, 32, VENUSAUR
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, HEADBUTT
	db 13, VINE_WHIP
	db 22, POISONPOWDER
	db 30, RAZOR_LEAF
	db 34, LIGHT_SCREEN
	db 38, GROWTH
	db 41, PETAL_DANCE
	db 46, SLEEP_POWDER
	db 54, SOLARBEAM
	db 0

ExeggutorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 28, STOMP
	db 0

LickitungEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, LICKILICKI
	db 0
; Learnset
	db 7, STOMP
	db 15, DISABLE
	db 19, LICK
	db 23, DEFENSE_CURL
	db 31, SLAM
	db 39, SCREECH
	db 43, DOUBLESLAP
	db 0

ExeggcuteEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, EXEGGUTOR
	db 0
; Learnset
	db 23, PSYBEAM
	db 25, REFLECT
	db 28, LEECH_SEED
	db 32, STUN_SPORE
	db 37, POISONPOWDER
	db 40, DREAM_EATER
	db 42, SOLARBEAM
	db 48, SLEEP_POWDER
	db 0

GrimerEvosMoves:
; Evolutions
	db EV_LEVEL, 38, MUK
	db 0
; Learnset
	db 20, LICK
	db 30, POISON_GAS
	db 33, MINIMIZE
	db 37, SLUDGE
	db 40, LICK
	db 42, HARDEN
	db 44, HAZE
	db 48, SCREECH
	db 55, ACID_ARMOR
	db 0

GengarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 29, HYPNOSIS
	db 38, DREAM_EATER
	db 51, LICK									;Added, so Gengar uses it in Pokemon Tower
	db 51, MEGA_DRAIN							;Added, so Gengar uses it in Pokemon Tower
	db 51, METRONOME							;Added, so Gengar uses it in Pokemon Tower
	db 51, EXPLOSION							;Added, so Gengar uses it in Pokemon Tower
	db 61, SLUDGE
	db 0

NidoranFEvosMoves:
; Evolutions
	db EV_LEVEL, 16, NIDORINA
	db 0
; Learnset
	db 8, SCRATCH
	db 14, POISON_STING
	db 21, TAIL_WHIP
	db 27, LOVELY_KISS
	db 29, BITE
	db 34, SUPERSONIC
	db 36, FURY_SWIPES
	db 43, DOUBLE_KICK
	db 0

NidoqueenEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, SCRATCH
	db 14, POISON_STING
	db 23, BODY_SLAM
	db 27, FIRE_PUNCH
	db 28, THUNDERPUNCH
	db 29, ICE_PUNCH
	db 0

CuboneEvosMoves:
; Evolutions
	db EV_LEVEL, 28, MAROWAK
	db 0
; Learnset
	db 10, DOUBLE_KICK
	db 18, SING
	db 25, LEER
	db 31, FOCUS_ENERGY
	db 33, THUNDERPUNCH
	db 37, FLAMETHROWER
	db 38, THRASH
	db 43, BONEMERANG
	db 46, RAGE
	db 0

RhyhornEvosMoves:
; Evolutions
	db EV_LEVEL, 42, RHYDON
	db 0
; Learnset
	db 15, ROAR
	db 30, STOMP
	db 35, TAIL_WHIP
	db 40, FURY_ATTACK
	db 45, HORN_DRILL
	db 50, LEER
	db 55, TAKE_DOWN
	db 0

LaprasEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, BITE
	db 12, AURORA_BEAM
	db 16, SING
	db 20, MIST
	db 25, BODY_SLAM
	db 31, CONFUSE_RAY
	db 38, ICE_BEAM
	db 42, DREAM_EATER
	db 46, HYDRO_PUMP
	db 0

ArcanineEvosMoves:
; Evolutions
	db 0
; Learnset
	db 66, BODY_SLAM							;Added, for wild Arcanines in Silph Co.
	db 0

MewEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, TRANSFORM
	db 20, MEGA_PUNCH
	db 30, METRONOME
	db 40, PSYCHIC_M
	db 0

GyaradosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 20, BITE
	db 25, DRAGON_RAGE
	db 28, WATERFALL
	db 32, LEER
	db 41, HYDRO_PUMP
	db 52, HYPER_BEAM
	db 65, THRASH
	db 0

ShellderEvosMoves:
; Evolutions
	db EV_ITEM, WATER_STONE, 1, CLOYSTER
	db 0
; Learnset
	db 18, SUPERSONIC
	db 23, CLAMP
	db 30, AURORA_BEAM
	db 39, LEER
	db 50, ICE_BEAM
	db 63, TWINEEDLE
	db 0

TentacoolEvosMoves:
; Evolutions
	db EV_LEVEL, 30, TENTACRUEL
	db 0
; Learnset
	db 7, SUPERSONIC
	db 13, WRAP
	db 18, POISON_STING
	db 22, WATER_GUN
	db 27, CONSTRICT
	db 33, BARRIER
	db 36, CONFUSE_RAY
	db 40, SCREECH
	db 41, AURORA_BEAM
	DB 42, HAZE
	db 48, HYDRO_PUMP
	db 0

GastlyEvosMoves:
; Evolutions
	db EV_LEVEL, 25, HAUNTER
	db 0
; Learnset
	db 27, HYPNOSIS
	db 35, DREAM_EATER
	db 52, EXPLOSION							;Added, so Gastly uses it in Pokemon Tower
	db 52, EXPLOSION							;Added, so Gastly uses it in Pokemon Tower
	db 52, EXPLOSION							;Added, so Gastly uses it in Pokemon Tower
	db 52, EXPLOSION							;Added, so Gastly uses it in Pokemon Tower
	db 57, SLUDGE
	db 0

ScytherEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, SCIZOR
	db 0
; Learnset
	db 17, LEER
	db 20, FOCUS_ENERGY
	db 24, DOUBLE_TEAM
	db 29, SLASH
	db 35, SWORDS_DANCE
	db 42, AGILITY
	db 50, LIGHT_SCREEN
	db 0

StaryuEvosMoves:
; Evolutions
	db EV_ITEM, WATER_STONE, 1, STARMIE
	db 0
; Learnset
	db 17, WATER_GUN
	db 22, HARDEN
	db 25, LIGHT_SCREEN							;Added for Misty's Staryu
	db 27, RECOVER
	db 32, SWIFT
	db 37, MINIMIZE
	db 42, LIGHT_SCREEN
	db 47, HYDRO_PUMP
	db 0

BlastoiseEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, BUBBLE
	db 9, CONFUSION
	db 15, WATER_GUN
	db 22, ICE_PUNCH
	db 24, BITE
	db 31, WITHDRAW
	db 42, SKULL_BASH
	db 47, HAZE
	db 50, MIST
	db 52, HYDRO_PUMP
	db 0

PinsirEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, ROCK_THROW
	db 25, SEISMIC_TOSS
	db 30, GUILLOTINE
	db 36, FOCUS_ENERGY
	db 39, LEECH_LIFE
	db 43, HARDEN
	db 49, SLASH
	db 54, SWORDS_DANCE
	db 0

TangelaEvosMoves:
; Evolutions
	db 0
; Learnset
	db 17, CONFUSION
	db 29, ABSORB
	db 32, POISONPOWDER
	db 36, STUN_SPORE
	db 39, SLEEP_POWDER
	db 45, SLAM
	db 49, GROWTH
	db 0

CrobatEvosMoves:
; Evolutions
	db 0
; Learnset
	db 66, TOXIC								;Added, for wild Crobats in Silph Co.
	db 0

MareepEvosMoves:
; Evolutions
	db EV_LEVEL, 15, FLAAFFY
	db 0
; Learnset
	db 9, THUNDERSHOCK
	db 16, THUNDER_WAVE
	db 23, SCREECH
	db 25, CONFUSE_RAY
	db 30, LIGHT_SCREEN
	db 37, THUNDER
	db 39, SAND_ATTACK
	db 0

GrowlitheEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, ARCANINE
	db 0
; Learnset
	db 18, EMBER
	db 23, LEER
	db 30, TAKE_DOWN
	db 39, AGILITY
	db 50, FLAMETHROWER
	db 57, FIRE_SPIN
	db 63, THRASH
	db 0

OnixEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, DIMONIX
	db 0
; Learnset
	db 15, BIND
	db 19, ROCK_THROW
	db 23, HEADBUTT
	db 25, RAGE
	db 29, ROAR
	db 33, SLAM
	db 39, SHARPEN
	db 43, HARDEN
	db 0

FearowEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, LEER
	db 15, FURY_ATTACK
	db 19, SONICBOOM
	db 25, MIRROR_MOVE
	db 34, DRILL_PECK
	db 39, SLAM
	db 43, AGILITY
	db 53, TRI_ATTACK
	db 0

PidgeyEvosMoves:
; Evolutions
	db EV_LEVEL, 18, PIDGEOTTO
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 12, QUICK_ATTACK
	db 17, HEADBUTT
	db 19, WHIRLWIND
	db 28, WING_ATTACK
	db 36, AGILITY
	db 44, MIRROR_MOVE
	db 0

SlowpokeEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, SLOWKING
	db EV_LEVEL, 37, SLOWBRO
	db 0
; Learnset
	db 18, DISABLE
	db 22, HEADBUTT
	db 27, GROWL
	db 33, WATER_GUN
	db 40, AMNESIA
	db 48, PSYCHIC_M
	db 0

KadabraEvosMoves:
; Evolutions
	db EV_ITEM, THUNDER_STONE, 1, ALAKAZAM
	db 0
; Learnset
	db 16, CONFUSION
	db 20, DISABLE
	db 27, PSYBEAM
	db 31, RECOVER
	db 32, AURORA_BEAM
	db 38, PSYCHIC_M
	db 42, REFLECT
	db 44, KINESIS
	db 0

GravelerEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, GOLEM
	db 0
; Learnset
	db 11, DEFENSE_CURL
	db 16, ROCK_THROW
	db 21, SELFDESTRUCT
	db 29, HARDEN
	db 36, EARTHQUAKE
	db 43, EXPLOSION
	db 50, SAND_ATTACK
	db 0

ChanseyEvosMoves:
; Evolutions
	db 0
; Learnset
	db 24, SING
	db 30, GROWL
	db 34, DREAM_EATER
	db 38, MINIMIZE
	db 44, DEFENSE_CURL
	db 48, LIGHT_SCREEN
	db 51, FLAMETHROWER
	db 54, DOUBLE_EDGE
	db 0

MachokeEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, MACHAMP
	db 0
; Learnset
	db 20, LOW_KICK
	db 25, LEER
	db 36, FOCUS_ENERGY
	db 44, SEISMIC_TOSS
	db 52, SUBMISSION
	db 55, LIGHT_SCREEN
	db 0

MrMimeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, CONFUSION
	db 23, LIGHT_SCREEN
	db 31, DOUBLESLAP
	db 35, PSYBEAM
	db 39, MEDITATE
	db 43, HYPNOSIS
	db 47, SUBSTITUTE
	db 55, CONFUSE_RAY
	db 63, MIRROR_MOVE
	db 0

HitmonleeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 33, ROLLING_KICK
	db 38, JUMP_KICK
	db 43, FOCUS_ENERGY
	db 48, HI_JUMP_KICK
	db 53, MEGA_KICK
	db 0

HitmonchanEvosMoves:
; Evolutions
	db 0
; Learnset
	db 28, ROLLING_KICK
	db 33, FIRE_PUNCH
	db 38, ICE_PUNCH
	db 43, THUNDERPUNCH
	db 48, MEGA_PUNCH
	db 50, DIZZY_PUNCH
	db 53, COUNTER
	db 0

ArbokEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, POISON_STING
	db 17, BITE
	db 27, GLARE
	db 36, SCREECH
	db 41, HAZE
	db 42, HEADBUTT
	db 47, ACID
	db 55, SLAM
	db 0

ParasectEvosMoves:
; Evolutions
	db 0
; Learnset
	db 13, STUN_SPORE
	db 20, LEECH_LIFE
	db 24, MEGA_DRAIN							;Added, so Parasect uses it in Mt.Moon
	db 30, SPORE
	db 39, SLASH
	db 44, LIGHT_SCREEN
	db 48, GROWTH
	db 51, COUNTER
	db 58, PSYBEAM
	db 65, SLUDGE
	db 0

PsyduckEvosMoves:
; Evolutions
	db EV_LEVEL, 33, GOLDUCK
	db 0
; Learnset
	db 20, HYPNOSIS
	db 28, TAIL_WHIP
	db 31, DISABLE
	db 36, CONFUSION
	db 39, LIGHT_SCREEN
	db 43, FURY_SWIPES
	db 47, PETAL_DANCE
	db 52, HYDRO_PUMP
	db 0

DrowzeeEvosMoves:
; Evolutions
	db EV_LEVEL, 26, HYPNO
	db 0
; Learnset
	db 12, DISABLE
	db 17, CONFUSION
	db 24, HEADBUTT
	db 27, PSYBEAM
	db 29, POISON_GAS
	db 32, PSYCHIC_M
	db 33, FIRE_PUNCH
	db 36, ICE_PUNCH
	db 37, MEDITATE
	db 39, THUNDERPUNCH
	db 0

GolemEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, DEFENSE_CURL
	db 16, ROCK_THROW
	db 21, SELFDESTRUCT
	db 29, HARDEN
	db 36, EARTHQUAKE
	db 43, EXPLOSION
	db 50, SAND_ATTACK
	db 0

FlaaffyEvosMoves:
; Evolutions
	db EV_LEVEL, 30, AMPHAROS
	db 0
; Learnset
	db 9, THUNDERSHOCK
	db 18, THUNDER_WAVE
	db 27, SCREECH
	db 32, CONFUSE_RAY
	db 36, LIGHT_SCREEN
	db 45, THUNDER
	db 49, SAND_ATTACK
	db 0

MagmarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 36, LEER
	db 37, THUNDERPUNCH
	db 38, KARATE_CHOP
	db 39, CONFUSE_RAY
	db 43, FIRE_PUNCH
	db 48, SMOKESCREEN
	db 52, SMOG
	db 54, THUNDERPUNCH
	db 55, FLAMETHROWER
	db 60, BARRIER
	db 0

AmpharosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, THUNDERSHOCK
	db 18, THUNDER_WAVE
	db 27, SCREECH
	db 30, THUNDERPUNCH
	db 38, CONFUSE_RAY
	db 42, LIGHT_SCREEN
	db 57, THUNDER
	db 64, SAND_ATTACK
	db 0

ElectabuzzEvosMoves:
; Evolutions
	db 0
; Learnset
	db 34, THUNDERSHOCK
	db 35, FIRE_PUNCH
	db 37, SCREECH
	db 38, ROLLING_KICK
	db 42, THUNDERPUNCH
	db 49, LIGHT_SCREEN
	db 52, FIRE_PUNCH
	db 54, THUNDER
	db 60, BARRIER
	db 0

MagnetonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, SONICBOOM
	db 25, THUNDERSHOCK
	db 29, SUPERSONIC
	db 38, THUNDER_WAVE
	db 46, SWIFT
	db 54, SCREECH
	db 57, TRI_ATTACK
	db 67, AGILITY
	db 74, LIGHT_SCREEN
	db 0

KoffingEvosMoves:
; Evolutions
	db EV_LEVEL, 35, WEEZING
	db 0
; Learnset
	db 17, POISON_GAS
	db 32, SLUDGE
	db 37, SMOKESCREEN
	db 38, PSYBEAM
	db 40, SELFDESTRUCT
	db 43, FLAMETHROWER
	db 45, HAZE
	db 46, SCREECH
	db 48, EXPLOSION
	db 49, PSYWAVE
	db 0

BellossomEvosMoves:
; Evolutions
	db 0
; Learnset
    db 55, SOLARBEAM
	db 0

MankeyEvosMoves:
; Evolutions
	db EV_LEVEL, 28, PRIMEAPE
	db 0
; Learnset
	db 15, KARATE_CHOP
	db 21, FURY_SWIPES
	db 27, FOCUS_ENERGY
	db 29, COMET_PUNCH
	db 33, SEISMIC_TOSS
	db 39, THRASH
	db 44, MEDITATE
	db 0

SeelEvosMoves:
; Evolutions
	db EV_LEVEL, 34, DEWGONG
	db 0
; Learnset
	db 30, GROWL
	db 35, AURORA_BEAM
	db 37, WATERFALL
	db 40, REST
	db 42, LICK
	db 45, TAKE_DOWN
	db 47, SLAM
	db 50, ICE_BEAM
	db 52, DISABLE
	db 0

DiglettEvosMoves:
; Evolutions
	db EV_LEVEL, 26, DUGTRIO
	db 0
; Learnset
	db 15, GROWL
	db 19, DIG
	db 24, SAND_ATTACK
	db 31, SLASH
	db 32, SCREECH
	db 40, EARTHQUAKE
	db 48, SLUDGE
	db 0

TaurosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, STOMP
	db 28, TAIL_WHIP
	db 35, LEER
	db 44, RAGE
	db 51, TAKE_DOWN
	db 0

SudowoodoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, LOW_KICK								;Added for Brock's Sudowoodo
	db 10, HARDEN
	db 19, LOW_KICK
	db 28, ROCK_SLIDE
	db 37, MEGA_KICK
	db 46, SLAM
	db 0

PolitoedEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, HYPNOSIS
	db 19, WATER_GUN
	db 0

HoppipEvosMoves:
; Evolutions
	db EV_LEVEL, 18, SKIPLOOM
	db 0
; Learnset
	db 5, TAIL_WHIP
	db 10, TACKLE
	db 13, POISONPOWDER
	db 15, STUN_SPORE
	db 17, SLEEP_POWDER
	db 20, LEECH_SEED
	db 22, ABSORB
	db 25, CONFUSION
	db 30, MEGA_DRAIN
	db 40, AGILITY
	db 0

FarfetchdEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, REEFETCHD
	db 0
; Learnset
	db 7, LEER
	db 15, FURY_ATTACK
	db 23, SWORDS_DANCE
	db 31, AGILITY
	db 39, SLASH
	db 47, RAZOR_LEAF
	db 55, MIRROR_MOVE
	db 0

VenonatEvosMoves:
; Evolutions
	db EV_LEVEL, 31, VENOMOTH
	db 0
; Learnset
	db 21, POISONPOWDER							;Moved from 24 to 21 to make room for Sludge at level 24
	db 24, SLUDGE
	db 27, LEECH_LIFE
	db 30, STUN_SPORE
	db 34, CONFUSE_RAY
	db 35, PSYBEAM
	db 37, NIGHT_SHADE
	db 38, SLEEP_POWDER
	db 43, PSYCHIC_M
	db 0

DragoniteEvosMoves:
; Evolutions
	db 0
; Learnset
	db  5, SUPERSONIC
	db 10, THUNDER_WAVE
	db 15, WATERFALL
	db 20, AGILITY
	db 35, SLAM
	db 40, FLAMETHROWER
	db 45, DRAGON_RAGE
	db 55, WING_ATTACK
	db 60, HYPER_BEAM
	db 65, LIGHT_SCREEN
	db 0

SkiploomEvosMoves:
; Evolutions
	db EV_LEVEL, 27, JUMPLUFF
	db 0
; Learnset
	db 5, TAIL_WHIP
	db 10, TACKLE
	db 13, POISONPOWDER
	db 15, STUN_SPORE
	db 17, SLEEP_POWDER
	db 22, LEECH_SEED
	db 26, ABSORB
	db 29, CONFUSION
	db 36, MEGA_DRAIN
	db 48, AGILITY
	db 0

JumpluffEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, TAIL_WHIP
	db 10, TACKLE
	db 13, POISONPOWDER
	db 15, STUN_SPORE
	db 17, SLEEP_POWDER
	db 22, LEECH_SEED
	db 26, ABSORB
	db 36, CONFUSION
	db 44, MEGA_DRAIN
	db 60, AGILITY
	db 0

LeafeonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 27, QUICK_ATTACK
	db 31, RAZOR_LEAF
	db 37, TAIL_WHIP
	db 40, BITE
	db 42, SWORDS_DANCE
	db 44, RAZOR_LEAF
	db 48, SING
	db 54, SOLARBEAM
	db 0

DoduoEvosMoves:
; Evolutions
	db EV_LEVEL, 31, DODRIO
	db 0
; Learnset
	db 20, GROWL
	db 24, FURY_ATTACK
	db 30, DRILL_PECK
	db 36, RAGE
	db 37, JUMP_KICK
	db 38, SUPERSONIC
	db 40, TRI_ATTACK
	db 44, AGILITY
	db 0

PoliwagEvosMoves:
; Evolutions
	db EV_LEVEL, 25, POLIWHIRL
	db 0
; Learnset
	db 16, HYPNOSIS
	db 19, WATER_GUN
	db 22, GROWTH
	db 25, DOUBLESLAP
	db 28, HAZE
	db 30, WATERFALL
	db 31, BODY_SLAM
	db 32, LOVELY_KISS
	db 38, AMNESIA
	db 45, HYDRO_PUMP
	db 0

JynxEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, LOVELY_KISS							;Added, so Jynx uses it on Route 11
	db 18, LICK
	db 23, DOUBLESLAP
	db 31, ICE_PUNCH
	db 35, CONFUSION
	db 39, BODY_SLAM
	db 47, THRASH
	db 58, BLIZZARD
	db 0

MoltresEvosMoves:
; Evolutions
	db 0
; Learnset
	db 44, LEER
	db 47, WING_ATTACK
	db 51, FLAMETHROWER
	db 55, AGILITY
	db 60, SKY_ATTACK
	db 0

ArticunoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 44, GUST
	db 47, HAZE
	db 51, BLIZZARD
	db 55, AGILITY
	db 60, MIST
	db 0

ZapdosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 44, ROAR
	db 47, DOUBLE_TEAM
	db 51, THUNDER
	db 55, AGILITY
	db 60, LIGHT_SCREEN
	db 0

DittoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

MeowthEvosMoves:
; Evolutions
	db EV_LEVEL, 28, PERSIAN
	db 0
; Learnset
	db 12, BITE
	db 17, PAY_DAY
	db 24, SCREECH
	db 33, FURY_SWIPES
	db 37, HYPNOSIS
	db 43, DREAM_EATER
	db 44, SLASH
	db 53, AMNESIA
	db 0

KrabbyEvosMoves:
; Evolutions
	db EV_LEVEL, 28, KINGLER
	db 0
; Learnset
	db 20, VICEGRIP
	db 25, GUILLOTINE
	db 30, STOMP
	db 35, CRABHAMMER
	db 40, HARDEN
	db 45, SLAM
	db 50, AGILITY
	db 0

WooperEvosMoves:
; Evolutions
	db EV_LEVEL, 20, QUAGSIRE
	db 0
; Learnset
	db 6, DEFENSE_CURL
	db 11, SLAM
	db 21, AMNESIA
	db 22, ICE_PUNCH
	db 31, EARTHQUAKE
	db 41, WATERFALL
	db 46, SLUDGE
	db 51, MIST
	db 56, HAZE
	db 61, RECOVER
	db 0

QuagsireEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, DEFENSE_CURL
	db 11, SLAM
	db 23, AMNESIA
	db 32, ICE_PUNCH
	db 35, EARTHQUAKE
	db 47, WATERFALL
	db 53, SLUDGE
	db 59, MIST
	db 66, HAZE
	db 73, RECOVER
	db 0

DunsparceEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, DEFENSE_CURL
	db 13, GLARE
	db 18, HEADBUTT
	db 26, BITE
	db 30, SCREECH
	db 38, TAKE_DOWN
	db 42, AGILITY
	db 0

VulpixEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, NINETALES
	db 0
; Learnset
	db 16, QUICK_ATTACK
	db 21, ROAR
	db 28, CONFUSE_RAY
	db 35, FLAMETHROWER
	db 39, DISABLE
	db 40, HYPNOSIS
	db 42, FIRE_SPIN
	db 0

NinetalesEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

PikachuEvosMoves:
; Evolutions
	db EV_ITEM, THUNDER_STONE, 1, RAICHU
	db 0
; Learnset
	db 9, THUNDER_WAVE
	db 16, QUICK_ATTACK
	db 26, SWIFT
	db 33, AGILITY
	db 38, DIZZY_PUNCH
	db 43, THUNDER
	db 0

RaichuEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

MisdreavusEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, SCREECH
	db 12, CONFUSE_RAY
	db 19, HEADBUTT
	db 27, PSYBEAM
	db 36, NIGHT_SHADE
	db 46, HYPNOSIS
	db 0

SlowkingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 18, DISABLE
	db 22, HEADBUTT
	db 27, GROWL
	db 33, WATER_GUN
	db 37, WITHDRAW
	db 44, AMNESIA
	db 55, PSYCHIC_M
	db 0

DratiniEvosMoves:
; Evolutions
	db EV_LEVEL, 30, DRAGONAIR
	db 0
; Learnset
	db  5, SUPERSONIC
	db 10, THUNDER_WAVE
	db 15, WATERFALL
	db 20, AGILITY
	db 30, SLAM
	db 35, FLAMETHROWER
	db 40, DRAGON_RAGE
	db 45, LIGHT_SCREEN
	db 50, HYPER_BEAM
	db 0

DragonairEvosMoves:
; Evolutions
	db EV_LEVEL, 55, DRAGONITE
	db 0
; Learnset
	db  5, SUPERSONIC
	db 10, THUNDER_WAVE
	db 15, WATERFALL
	db 20, AGILITY
	db 35, SLAM
	db 40, FLAMETHROWER
	db 45, DRAGON_RAGE
	db 55, HYPER_BEAM
	db 60, LIGHT_SCREEN
	db 0

KabutoEvosMoves:
; Evolutions
	db EV_LEVEL, 40, KABUTOPS
	db 0
; Learnset
	db 12, ROCK_THROW
	db 22, AURORA_BEAM
	db 34, ABSORB
	db 39, SLASH
	db 42, ROCK_THROW
	db 43, MEGA_DRAIN
	db 44, LEER
	db 46, SAND_ATTACK
	db 48, DIG
	db 49, HYDRO_PUMP
	db 0

KabutopsEvosMoves:
; Evolutions
	db 0
; Learnset
	db 34, ABSORB
	db 39, SLASH
	db 46, LEER
	db 53, HYDRO_PUMP
	db 0

HorseaEvosMoves:
; Evolutions
	db EV_LEVEL, 32, SEADRA
	db 0
; Learnset
	db 19, SMOKESCREEN
	db 24, LEER
	db 30, WATER_GUN
	db 35, HAZE
	db 37, AGILITY
	db 39, WATERFALL
	db 45, HYDRO_PUMP
	db 49, DRAGON_RAGE
	db 0

SeadraEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, KINGDRA
	db 0
; Learnset
	db 19, SMOKESCREEN
	db 24, LEER
	db 30, WATER_GUN
	db 41, AGILITY
	db 50, HAZE
	db 52, HYDRO_PUMP
	db 54, WATERFALL
	db 64, DRAGON_RAGE
	db 0

GligarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, SAND_ATTACK
	db 13, HARDEN
	db 20, QUICK_ATTACK
	db 28, WING_ATTACK
	db 36, SLASH
	db 44, SCREECH
	db 52, GUILLOTINE
	db 60, SLUDGE
	db 0

DimonixEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

SandshrewEvosMoves:
; Evolutions
	db EV_LEVEL, 22, SANDSLASH
	db 0
; Learnset
	db 10, SAND_ATTACK
	db 17, SLASH
	db 24, POISON_STING
	db 25, SUPER_FANG
	db 31, SWIFT
	db 38, FURY_SWIPES
	db 0

SandslashEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, SAND_ATTACK
	db 17, SLASH
	db 27, POISON_STING
	db 36, SWIFT
	db 40, SUPER_FANG
	db 47, FURY_SWIPES
	db 0

OmanyteEvosMoves:
; Evolutions
	db EV_LEVEL, 40, OMASTAR
	db 0
; Learnset
	db 12, AURORA_BEAM
	db 22, CONSTRICT
	db 34, HORN_ATTACK
	db 39, LEER
	db 42, CONSTRICT
	db 44, ROCK_THROW
	db 45, SUPERSONIC
	db 46, SPIKE_CANNON
	db 48, HAZE
	db 53, HYDRO_PUMP
	db 0

OmastarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 34, HORN_ATTACK
	db 39, LEER
	db 44, SPIKE_CANNON
	db 49, HYDRO_PUMP
	db 0

JigglypuffEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, WIGGLYTUFF
	db 0
; Learnset
	db 9, POUND
	db 14, DISABLE
	db 19, DEFENSE_CURL
	db 24, DOUBLESLAP
	db 27, FLAMETHROWER
	db 29, REST
	db 30, HEADBUTT
	db 34, BODY_SLAM
	db 39, DOUBLE_EDGE
	db 42, DREAM_EATER
	db 46, PETAL_DANCE
	db 0

WigglytuffEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

EeveeEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, FLAREON
	db EV_ITEM, THUNDER_STONE, 1, JOLTEON
	db EV_ITEM, WATER_STONE, 1, VAPOREON
	db EV_ITEM, LEAF_STONE, 1, LEAFEON
	db 0
; Learnset
	db 27, QUICK_ATTACK
	db 31, TAIL_WHIP
	db 33, HEADBUTT
	db 37, BITE
	db 41, GROWTH
	db 45, TAKE_DOWN
	db 47, DOUBLE_KICK
	db 49, SING
	db 0

FlareonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 27, QUICK_ATTACK
	db 31, EMBER
	db 37, TAIL_WHIP
	db 40, BITE
	db 42, LEER
	db 44, FIRE_SPIN
	db 48, RAGE
	db 54, FLAMETHROWER
	db 62, SMOG
	db 0

JolteonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 27, QUICK_ATTACK
	db 31, THUNDERSHOCK
	db 37, TAIL_WHIP
	db 40, THUNDER_WAVE
	db 42, DOUBLE_KICK
	db 44, AGILITY
	db 48, PIN_MISSILE
	db 54, THUNDER
	db 62, LIGHT_SCREEN
	db 0

VaporeonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 27, QUICK_ATTACK
	db 31, WATER_GUN
	db 37, TAIL_WHIP
	db 40, BITE
	db 42, ACID_ARMOR
	db 44, HAZE
	db 48, MIST
	db 54, HYDRO_PUMP
	db 62, WATERFALL
	db 0

MachopEvosMoves:
; Evolutions
	db EV_LEVEL, 28, MACHOKE
	db 0
; Learnset
	db 20, LOW_KICK
	db 25, LEER
	db 32, FOCUS_ENERGY
	db 39, SEISMIC_TOSS
	db 40, LIGHT_SCREEN
	db 46, SUBMISSION
	db 0

ZubatEvosMoves:
; Evolutions
	db EV_LEVEL, 22, GOLBAT
	db 0
; Learnset
	db 10, SUPERSONIC
	db 15, BITE
	db 21, CONFUSE_RAY
	db 28, WING_ATTACK
	db 36, HAZE
	db 45, SLUDGE
	db 0

EkansEvosMoves:
; Evolutions
	db EV_LEVEL, 22, ARBOK
	db 0
; Learnset
	db 10, POISON_STING
	db 17, BITE
	db 24, GLARE
	db 26, HAZE
	db 31, SCREECH
	db 36, HEADBUTT
	db 38, ACID
	db 40, SLAM
	db 0

ParasEvosMoves:
; Evolutions
	db EV_LEVEL, 24, PARASECT
	db 0
; Learnset
	db 13, STUN_SPORE
	db 20, LEECH_LIFE
	db 24, MEGA_DRAIN							;Added, so Paras gets Mega Drain early-game
	db 27, SPORE
	db 29, LIGHT_SCREEN
	db 34, SLASH
	db 36, COUNTER
	db 41, GROWTH
	db 43, PSYBEAM
	db 50, SLUDGE
	db 0

PoliwhirlEvosMoves:
; Evolutions
	db EV_ITEM, WATER_STONE, 1, POLIWRATH
	db EV_ITEM, MIST_STONE, 1, POLITOED
	db 0
; Learnset
	db 16, HYPNOSIS
	db 19, WATER_GUN
	db 22, GROWTH
	db 26, DOUBLESLAP
	db 28, ICE_PUNCH
	db 33, BODY_SLAM
	db 41, AMNESIA
	db 43, HAZE
	db 45, WATERFALL
	db 47, LOVELY_KISS
	db 49, HYDRO_PUMP
	db 0

PoliwrathEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, HYPNOSIS
	db 19, WATER_GUN
	db 0

WeedleEvosMoves:
; Evolutions
	db EV_LEVEL, 7, KAKUNA
	db 0
; Learnset
	db 6, REST
	db 0

KakunaEvosMoves:
; Evolutions
	db EV_LEVEL, 10, BEEDRILL
	db 0
; Learnset
	db 7, HARDEN
	db 12, LIGHT_SCREEN
	db 19, WITHDRAW
	db 0

BeedrillEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, FURY_ATTACK
	db 16, FOCUS_ENERGY
	db 20, TWINEEDLE
	db 25, RAGE
	db 29, SLUDGE
	db 30, PIN_MISSILE
	db 35, AGILITY
	db 47, PETAL_DANCE
	db 59, DISABLE
	db 0

QwilfishEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, HARDEN
	db 10, MINIMIZE
	db 19, WATER_GUN
	db 28, PIN_MISSILE
	db 37, TAKE_DOWN
	db 46, HYDRO_PUMP
	db 55, SLUDGE
	db 0

DodrioEvosMoves:
; Evolutions
	db 0
; Learnset
	db 20, GROWL
	db 24, FURY_ATTACK
	db 30, DRILL_PECK
	db 39, RAGE
	db 45, TRI_ATTACK
	db 49, JUMP_KICK
	db 50, SUPERSONIC
	db 51, AGILITY
	db 0

PrimeapeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, KARATE_CHOP
	db 21, FURY_SWIPES
	db 27, FOCUS_ENERGY
	db 37, SEISMIC_TOSS
	db 44, COMET_PUNCH
	db 46, THRASH
	db 59, MEDITATE
	db 0

DugtrioEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, GROWL
	db 19, DIG
	db 24, SAND_ATTACK
	db 31, DIG									;added such that wild Dugtrio in Diglett's Cave know Dig
	db 35, SLASH
	db 47, SCREECH
	db 47, EARTHQUAKE
	db 55, TRI_ATTACK
	db 63, SLUDGE
	db 0

VenomothEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, POISONPOWDER							;Moved from 24 to 21 to make room for Sludge at level 24	
	db 24, SLUDGE
	db 27, LEECH_LIFE
	db 30, STUN_SPORE
	db 38, PSYBEAM
	db 43, SLEEP_POWDER
	db 49, CONFUSE_RAY
	db 50, PSYCHIC_M
	db 53, NIGHT_SHADE
	db 0

DewgongEvosMoves:
; Evolutions
	db 0
; Learnset
	db 30, GROWL
	db 35, AURORA_BEAM
	db 44, REST
	db 47, WATERFALL
	db 50, TAKE_DOWN
	db 55, LICK
	db 56, ICE_BEAM
	db 60, SLAM
	db 65, DISABLE
	db 0

ScizorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

HeracrossEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, HORN_ATTACK
	db 12, HARDEN
	db 19, FURY_ATTACK
	db 27, COUNTER
	db 35, TAKE_DOWN
	db 44, LOW_KICK
	db 54, PIN_MISSILE
	db 0

CaterpieEvosMoves:
; Evolutions
	db EV_LEVEL, 7, METAPOD
	db 0
; Learnset
	db 6, COUNTER
	db 0

MetapodEvosMoves:
; Evolutions
	db EV_LEVEL, 10, BUTTERFREE
	db 0
; Learnset
	db 7, HARDEN
	db 12, WITHDRAW
	db 19, LIGHT_SCREEN
	db 0

ButterfreeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, CONFUSION
	db 15, POISONPOWDER
	db 16, STUN_SPORE
	db 17, SLEEP_POWDER
	db 21, SUPERSONIC
	db 26, WHIRLWIND
	db 32, PSYBEAM
	db 41, SPORE
	db 52, DREAM_EATER
	db 65, AMNESIA
	db 0

MachampEvosMoves:
; Evolutions
	db 0
; Learnset
	db 20, LOW_KICK
	db 25, LEER
	db 36, FOCUS_ENERGY
	db 44, SEISMIC_TOSS
	db 52, SUBMISSION
	db 0

SneaselEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, QUICK_ATTACK
	db 17, SCREECH
	db 25, ICE_PUNCH
	db 33, FURY_SWIPES
	db 41, AGILITY
	db 45, HEADBUTT
	db 49, SLASH
	db 57, BITE
	db 65, LOW_KICK
	db 0

GolduckEvosMoves:
; Evolutions
	db 0
; Learnset
	db 28, TAIL_WHIP
	db 31, DISABLE
	db 35, HYPNOSIS
	db 39, CONFUSION
	db 44, LIGHT_SCREEN
	db 48, FURY_SWIPES
	db 58, PETAL_DANCE
	db 59, HYDRO_PUMP
	db 0

HypnoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, DISABLE
	db 17, CONFUSION
	db 24, HEADBUTT
	db 30, PSYBEAM
	db 33, POISON_GAS
	db 37, PSYCHIC_M
	db 43, MEDITATE
	db 48, FIRE_PUNCH
	db 51, ICE_PUNCH
	db 54, THUNDERPUNCH
	db 0

GolbatEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, CROBAT
	db 0
; Learnset
	db 10, LEECH_LIFE								;Removed Supersonic for Leech Life, for wild Golbat in Mt.Moon
	db 15, BITE
	db 21, CONFUSE_RAY
	db 32, WING_ATTACK
	db 43, HAZE
	db 56, SLUDGE
	db 66, TOXIC									;Added, for wild Golbats in Silph Co.
	db 66, BITE										;Added, for wild Golbats in Silph Co.
	db 66, CONFUSE_RAY								;Added, for wild Golbats in Silph Co.
	db 0

MewtwoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 63, BARRIER
	db 66, PSYCHIC_M
	db 70, RECOVER
	db 75, MIST
	db 81, AMNESIA
	db 88, SHADOW_BALL
	db 0

SnorlaxEvosMoves:
; Evolutions
	db 0
; Learnset
	db 35, BODY_SLAM
	db 41, HARDEN
	db 48, DOUBLE_EDGE
	db 56, HYPER_BEAM
	db 60, REST										;Added, so Snorlax uses it on Route 12 and Route 16
	db 65, LOVELY_KISS
	db 0

MagikarpEvosMoves:
; Evolutions
	db EV_LEVEL, 20, GYARADOS
	db 0
; Learnset
	db 15, TACKLE
	db 0

TeddiursaEvosMoves:
; Evolutions
	db EV_LEVEL, 30, URSARING
	db 0
; Learnset
	db 8, LICK
	db 15, FURY_SWIPES
	db 22, DEFENSE_CURL
	db 29, REST
	db 36, SLASH
	db 38, LICK
	db 43, FOCUS_ENERGY
	db 50, THRASH
	db 0

UrsaringEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, LICK
	db 15, FURY_SWIPES
	db 22, DEFENSE_CURL
	db 29, REST
	db 39, SLASH
	db 41, LICK
	db 49, FOCUS_ENERGY
	db 59, THRASH
	db 0

MukEvosMoves:
; Evolutions
	db 0
; Learnset
	db 20, LICK
	db 30, POISON_GAS
	db 33, MINIMIZE
	db 37, SLUDGE
	db 45, HARDEN
	db 53, SCREECH
	db 57, LICK
	db 60, ACID_ARMOR
	db 63, HAZE
	db 0

SlugmaEvosMoves:
; Evolutions
	db EV_LEVEL, 38, MAGCARGO
	db 0
; Learnset
	db 8, EMBER
	db 15, ROCK_THROW
	db 22, HARDEN
	db 28, ACID_ARMOR
	db 35, SMOKESCREEN
	db 36, FLAMETHROWER
	db 43, ROCK_SLIDE
	db 50, THUNDER_WAVE
	db 56, RECOVER
	db 58, AMNESIA
	db 0

KinglerEvosMoves:
; Evolutions
	db 0
; Learnset
	db 20, VICEGRIP
	db 25, GUILLOTINE
	db 34, STOMP
	db 42, CRABHAMMER
	db 49, HARDEN
	db 60, SLAM
	db 65, AGILITY
	db 0

CloysterEvosMoves:
; Evolutions
	db 0
; Learnset
	db 50, SPIKE_CANNON
	db 0

MagcargoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, EMBER
	db 15, ROCK_THROW
	db 22, HARDEN
	db 28, ACID_ARMOR
	db 35, SMOKESCREEN
	db 36, FLAMETHROWER
	db 48, ROCK_SLIDE
	db 60, THUNDER_WAVE
	db 68, RECOVER
	db 76, AMNESIA
	db 0

ElectrodeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 17, SONICBOOM
	db 22, SELFDESTRUCT
	db 29, LIGHT_SCREEN								;Added, so Electrode uses it on Route 11
	db 29, THUNDER_WAVE								;Added, so Electrode uses it on Route 11
	db 29, THUNDERBOLT								;Added, so Electrode uses it on Route 11
	db 29, EXPLOSION								;Added, so Electrode uses it on Route 11
	db 40, SWIFT
	db 41, THUNDERBOLT								;Added, so Electrode uses it at the Power Plant
	db 41, THUNDER_WAVE								;Added, so Electrode uses it at the Power Plant
	db 41, SUBSTITUTE								;Added, so Electrode uses it at the Power Plant
	db 41, EXPLOSION								;Added, so Electrode uses it at the Power Plant
	db 50, EXPLOSION
	db 54, BARRAGE
	db 66, THUNDERBOLT								;Added, for wild Electrodes in Silph Co.
	db 66, THUNDERBOLT								;Added, for wild Electrodes in Silph Co.
	db 66, THUNDER_WAVE								;Added, for wild Electrodes in Silph Co.
	db 66, EXPLOSION								;Added, for wild Electrodes in Silph Co.
	db 68, AMNESIA
	db 0

ClefableEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

WeezingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 31, POISON_GAS
	db 32, SLUDGE
	db 39, SMOKESCREEN
	db 43, SELFDESTRUCT
	db 49, HAZE
	db 52, PSYBEAM
	db 53, EXPLOSION
	db 58, FLAMETHROWER
	db 63, SCREECH
	db 68, PSYWAVE
	db 0

PersianEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, BITE
	db 17, PAY_DAY
	db 24, SCREECH
	db 31, BODY_SLAM								;Added, so Persian uses it on Route 11
	db 37, FURY_SWIPES
	db 51, SLASH
	db 52, HYPNOSIS
	db 58, DREAM_EATER
	db 68, AMNESIA
	db 0

MarowakEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, DOUBLE_KICK
	db 18, SING
	db 25, LEER
	db 33, FOCUS_ENERGY
	db 41, THRASH
	db 45, THUNDERPUNCH
	db 48, BONEMERANG
	db 52, FLAMETHROWER
	db 55, RAGE
	db 0

SwinubEvosMoves:
; Evolutions
	db EV_LEVEL, 33, PILOSWINE
	db 0
; Learnset
	db 10, BITE
	db 19, DEFENSE_CURL
	db 28, TAKE_DOWN
	db 37, MIST
	db 46, BLIZZARD
	db 55, AMNESIA
	db 0

HaunterEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, GENGAR
	db 0
; Learnset
	db 29, HYPNOSIS
	db 38, DREAM_EATER
	db 61, SLUDGE
	db 0

AbraEvosMoves:
; Evolutions
	db EV_LEVEL, 16, KADABRA
	db 0
; Learnset
	db 17, AURORA_BEAM
	db 29, KINESIS
	db 0

AlakazamEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, CONFUSION
	db 20, DISABLE
	db 27, PSYBEAM
	db 31, RECOVER
	db 38, PSYCHIC_M
	db 42, REFLECT
	db 0

PidgeottoEvosMoves:
; Evolutions
	db EV_LEVEL, 36, PIDGEOT
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 12, QUICK_ATTACK
	db 21, WHIRLWIND
	db 27, HEADBUTT
	db 31, WING_ATTACK
	db 40, AGILITY
	db 49, MIRROR_MOVE
	db 0

PidgeotEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 12, QUICK_ATTACK
	db 21, WHIRLWIND
	db 31, WING_ATTACK
	db 37, HEADBUTT
	db 44, AGILITY
	db 54, MIRROR_MOVE
	db 0

StarmieEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

BulbasaurEvosMoves:
; Evolutions
	db EV_LEVEL, 16, IVYSAUR
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, HEADBUTT
	db 13, VINE_WHIP
	db 20, POISONPOWDER
	db 24, LIGHT_SCREEN
	db 27, RAZOR_LEAF
	db 31, PETAL_DANCE
	db 34, GROWTH
	db 41, SLEEP_POWDER
	db 48, SOLARBEAM
	db 0

VenusaurEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, HEADBUTT
	db 13, VINE_WHIP
	db 22, POISONPOWDER
	db 30, RAZOR_LEAF
	db 43, GROWTH
	db 44, LIGHT_SCREEN
	db 51, PETAL_DANCE
	db 55, SLEEP_POWDER
	db 65, SOLARBEAM
	db 0

TentacruelEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, SUPERSONIC
	db 13, WRAP
	db 18, POISON_STING
	db 22, WATER_GUN
	db 27, CONSTRICT
	db 35, BARRIER
	db 43, SCREECH
	db 50, HYDRO_PUMP
	db 51, CONFUSE_RAY
	db 55, AURORA_BEAM
	db 56, HAZE
	db 0

PiloswineEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, BITE
	db 19, DEFENSE_CURL
	db 28, TAKE_DOWN
	db 33, FURY_ATTACK
	db 42, MIST
	db 56, BLIZZARD
	db 70, AMNESIA
	db 0

GoldeenEvosMoves:
; Evolutions
	db EV_LEVEL, 33, SEAKING
	db 0
; Learnset
	db 19, SUPERSONIC
	db 24, HORN_ATTACK
	db 30, FURY_ATTACK
	db 34, PSYBEAM
	db 37, WATERFALL
	db 41, HYDRO_PUMP
	db 45, HORN_DRILL
	db 48, HAZE
	db 54, AGILITY
	db 55, SWORDS_DANCE
	db 0

SeakingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 19, SUPERSONIC
	db 24, HORN_ATTACK
	db 30, FURY_ATTACK
	db 39, WATERFALL
	db 48, HORN_DRILL
	db 49, PSYBEAM
	db 54, AGILITY
	db 56, HYDRO_PUMP
	db 63, HAZE
	db 70, SWORDS_DANCE
	db 0

CorsolaEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, HARDEN
	db 13, BUBBLE
	db 19, RECOVER
	db 25, BUBBLEBEAM
	db 31, SPIKE_CANNON
	db 34, HEADBUTT
	db 37, CONFUSE_RAY
	db 43, AMNESIA
	db 49, RECOVER
	db 0

HoundourEvosMoves:
; Evolutions
	db EV_LEVEL, 24, HOUNDOOM
	db 0
; Learnset
	db 7, ROAR
	db 13, SMOG
	db 20, BITE
	db 27, FIRE_SPIN
	db 35, FLAMETHROWER
	db 43, SUPER_FANG
	db 52, SLUDGE
	db 0

HoundoomEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, ROAR
	db 13, SMOG
	db 20, BITE
	db 30, FIRE_SPIN
	db 41, FLAMETHROWER
	db 52, SUPER_FANG
	db 65, SLUDGE
	db 0

KingdraEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

PonytaEvosMoves:
; Evolutions
	db EV_LEVEL, 40, RAPIDASH
	db 0
; Learnset
	db 30, TAIL_WHIP
	db 32, STOMP
	db 33, HEADBUTT
	db 35, GROWL
	db 38, PAY_DAY
	db 39, FIRE_SPIN
	db 41, LOW_KICK
	db 42, DOUBLE_KICK
	db 43, TAKE_DOWN
	db 44, THRASH
	db 48, AGILITY
	db 50, HYPNOSIS
	db 0

RapidashEvosMoves:
; Evolutions
	db 0
; Learnset
	db 30, TAIL_WHIP
	db 32, STOMP
	db 33, HEADBUTT
	db 35, GROWL
	db 38, PAY_DAY
	db 39, FIRE_SPIN
	db 47, TAKE_DOWN
	db 55, AGILITY
	db 56, LOW_KICK
	db 57, DOUBLE_KICK
	db 59, THRASH
	db 65, HYPNOSIS
	db 0

RattataEvosMoves:
; Evolutions
	db EV_LEVEL, 20, RATICATE
	db 0
; Learnset
	db 7, QUICK_ATTACK
	db 14, HYPER_FANG
	db 21, BITE
	db 23, FOCUS_ENERGY
	db 34, SUPER_FANG
	db 0

RaticateEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, QUICK_ATTACK
	db 14, HYPER_FANG
	db 27, FOCUS_ENERGY
	db 36, BITE
	db 41, SUPER_FANG
	db 0

NidorinoEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, NIDOKING
	db 0
; Learnset
	db 8, HORN_ATTACK
	db 14, POISON_STING
	db 23, FOCUS_ENERGY
	db 32, FURY_ATTACK
	db 41, HORN_DRILL
	db 42, SUPERSONIC
	db 50, DOUBLE_KICK
	db 51, LOVELY_KISS
	db 0

NidorinaEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, NIDOQUEEN
	db 0
; Learnset
	db 8, SCRATCH
	db 14, POISON_STING
	db 23, TAIL_WHIP
	db 32, BITE
	db 41, FURY_SWIPES
	db 42, LOVELY_KISS
	db 50, DOUBLE_KICK
	db 51, SUPERSONIC
	db 0

GeodudeEvosMoves:
; Evolutions
	db EV_LEVEL, 25, GRAVELER
	db 0
; Learnset
	db 11, DEFENSE_CURL
	db 16, ROCK_THROW
	db 21, SELFDESTRUCT
	db 26, HARDEN
	db 31, EARTHQUAKE
	db 36, EXPLOSION
	db 41, SAND_ATTACK
	db 0

PorygonEvosMoves:
; Evolutions
	db EV_ITEM, MIST_STONE, 1, PORYGON2
	db 0
; Learnset
	db 23, PSYBEAM
	db 28, RECOVER
	db 31, BARRIER
	db 35, AGILITY
	db 42, TRI_ATTACK
	db 0

AerodactylEvosMoves:
; Evolutions
	db 0
; Learnset
	db 33, SUPERSONIC
	db 35, ROCK_THROW
	db 38, BITE
	db 45, TAKE_DOWN
	db 54, HYPER_BEAM
	db 65, EARTHQUAKE
	db 0

PhanpyEvosMoves:
; Evolutions
	db EV_LEVEL, 25, DONPHAN
	db 0
; Learnset
	db 9, DEFENSE_CURL
	db 17, HEADBUTT
	db 25, TAKE_DOWN
	db 33, SLAM
	db 41, FOCUS_ENERGY
	db 49, DOUBLE_EDGE
	db 0

MagnemiteEvosMoves:
; Evolutions
	db EV_LEVEL, 30, MAGNETON
	db 0
; Learnset
	db 21, SONICBOOM
	db 25, THUNDERSHOCK
	db 29, SUPERSONIC
	db 35, THUNDER_WAVE
	db 41, SWIFT
	db 47, SCREECH
	db 52, AGILITY
	db 59, LIGHT_SCREEN
	db 0

DonphanEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, DEFENSE_CURL
	db 17, HEADBUTT
	db 25, FURY_ATTACK
	db 33, SLAM
	db 41, FOCUS_ENERGY
	db 49, EARTHQUAKE
	db 0

Porygon2EvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

CharmanderEvosMoves:
; Evolutions
	db EV_LEVEL, 16, CHARMELEON
	db 0
; Learnset
	db 9, EMBER
	db 11, BITE
	db 15, LEER
	db 17, FIRE_PUNCH
	db 22, RAGE
	db 24, SMOKESCREEN
	db 30, SLASH
	db 38, FLAMETHROWER
	db 46, FIRE_SPIN
	db 0

SquirtleEvosMoves:
; Evolutions
	db EV_LEVEL, 16, WARTORTLE
	db 0
; Learnset
	db 8, BUBBLE
	db 9, CONFUSION
	db 15, WATER_GUN
	db 17, ICE_PUNCH
	db 22, BITE
	db 27, HAZE
	db 28, WITHDRAW
	db 30, MIST
	db 35, SKULL_BASH
	db 42, HYDRO_PUMP
	db 0

CharmeleonEvosMoves:
; Evolutions
	db EV_LEVEL, 36, CHARIZARD
	db 0
; Learnset
	db 9, EMBER
	db 11, BITE
	db 15, LEER
	db 22, FIRE_PUNCH
	db 24, RAGE
	db 33, SLASH
	db 34, SMOKESCREEN
	db 42, FLAMETHROWER
	db 56, FIRE_SPIN
	db 0

WartortleEvosMoves:
; Evolutions
	db EV_LEVEL, 36, BLASTOISE
	db 0
; Learnset
	db 8, BUBBLE
	db 9, CONFUSION
	db 15, WATER_GUN
	db 22, ICE_PUNCH
	db 24, BITE
	db 31, WITHDRAW
	db 37, HAZE
	db 39, SKULL_BASH
	db 40, MIST
	db 47, HYDRO_PUMP
	db 0

CharizardEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, EMBER
	db 11, BITE
	db 15, LEER
	db 22, FIRE_PUNCH
	db 24, RAGE
	db 36, SLASH
	db 38, WING_ATTACK
	db 44, SMOKESCREEN
	db 46, FLAMETHROWER
	db 55, FIRE_SPIN
	db 0
	
LickilickiEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

ReefetchdEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

LugiaEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, HEADBUTT
	db 22, GUST
	db 33, RECOVER
	db 44, HYDRO_PUMP
	db 55, WATERFALL
	db 66, SWIFT
	db 77, WHIRLWIND
	db 88, LIGHT_SCREEN
	db 99, AEROBLAST
	db 0

HoohEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, SKY_ATTACK
	db 22, GUST
	db 33, RECOVER
	db 44, FIRE_BLAST
	db 55, FLAMETHROWER
	db 66, SWIFT
	db 77, WHIRLWIND
	db 88, LIGHT_SCREEN
	db 99, SACRED_FIRE
	db 0

OddishEvosMoves:
; Evolutions
	db EV_LEVEL, 21, GLOOM
	db 0
; Learnset
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 19, SLEEP_POWDER
	db 22, SLUDGE
	db 23, LEECH_SEED
	db 24, ACID
	db 33, PETAL_DANCE
	db 46, SOLARBEAM
	db 0

GloomEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, VILEPLUME
	db EV_ITEM, MIST_STONE, 1, BELLOSSOM
	db 0
; Learnset
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 19, SLEEP_POWDER
	db 28, ACID
	db 37, SLUDGE
	db 38, PETAL_DANCE
	db 39, LEECH_SEED
	db 52, SOLARBEAM
	db 0

VileplumeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 19, SLEEP_POWDER
	db 0

BellsproutEvosMoves:
; Evolutions
	db EV_LEVEL, 21, WEEPINBELL
	db 0
; Learnset
	db 13, WRAP
	db 15, POISONPOWDER
	db 18, SLEEP_POWDER
	db 21, STUN_SPORE
	db 25, LEECH_LIFE
	db 26, ACID
	db 33, RAZOR_LEAF
	db 42, SLAM
	db 0

WeepinbellEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, VICTREEBEL
	db 0
; Learnset
	db 13, WRAP
	db 15, POISONPOWDER
	db 18, SLEEP_POWDER
	db 23, STUN_SPORE
	db 29, ACID
	db 38, RAZOR_LEAF
	db 40, LEECH_LIFE
	db 49, SLAM
	db 0

VictreebelEvosMoves:
; Evolutions
	db 0
; Learnset
	db 13, WRAP
	db 15, POISONPOWDER
	db 18, SLEEP_POWDER
	db 0

ArmMewtwoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 63, BARRIER
	db 66, PSYCHIC_M
	db 70, RECOVER
	db 75, MIST
	db 81, AMNESIA
	db 95, COUNTER ;placeholder
	db 0
	
VenustoiseEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

MissingNoC1EvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

FossilKabutopsEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

FossilAerodactylEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

MonGhostEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0